    ...
    <div class="container-fluid">
      ...
      <div class="goods-list-box">
        <div class="goods-list clearfix">
          <c:forEach items="${productVOPage.records}" var="product">
            <div class="goods-item">
              <div class="figure figure-img"><a title="${product.introduce}" href="${ctx}/detail/${product.productId}"><img src="${ctximg}/${product.picImg}" title="${product.introduce}" width="200" height="200" alt="${product.name}" /></a></div>
              <p class="desc"></p>
              <h2 class="title"><a title="${product.name}" href="${ctx}/detail/${product.productId}">${product.name}</a></h2>
              <p class="price">${product.showPrice}元</p>
              <div class="thumbs">
                <ul class="thumb-list clearfix">
                  <li data-gid="${product.productId}"><img src="${ctximg}/${product.picImg}" width="34" height="34" title="${product.introduce}" alt="${product.name}" /></li>
                </ul>
                <p class="desc"> ${product.introduce} </p>
              </div>
              <div class="actions clearfix"> <a class="btn-like J_likeGoods" data-pid="${product.productId}" onclick="add_favorite(this);" href="javascript: void(0);"><i class="glyphicon glyphicon-heart-empty"></i> <span>喜欢</span></a> <a class="btn-buy J_buyGoods" data-gid="${product.productId}" href="${ctx}/detail/${product.productId}"><span>加入购物车</span> <i class="glyphicon glyphicon-shopping-cart"></i></a> </div>
              <div class="flags">
        ...
        <div class="xm-pagenavi">
          <c:if test="${productVOPage.total gt productVOPage.size and not empty productVOPage.records}">
            <div id="pager" data-pager-href="${ctx}/list?categoryId=${category.categoryId}&sort=${sort}&page=" data-pager-totalPage="${totalPage}" data-pager-current="${productVOPage.current}"></div>
          </c:if>
        </div>
      </div>
    </div>
