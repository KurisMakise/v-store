  ...
  <div class="container-fluid">
    <div class="row">
      <div class="span14 goods-question-list-block">
        <div class="goods-question-order-block"> <a href="${ctx}/question/list?productId=${productVO.productId}&sort=1" class="${sort eq 1 ? 'current':''} J_questionHelp">最有帮助</a> <span class="sep">|</span> <a href="${ctx}/question/list?productId=${productVO.productId}&sort=0" class="${sort eq 0 ? 'current':''} J_questionNew">最新</a> </div>
        ...
        <ul class="goods-question-list-detail" id="J_goodsQuestionBlock">
          <c:forEach items="${questionVOPage.records}" var="question">
            <li data-id="${question.questionId}">
              <div class="left-hand float">
                <div class="hand-block J_questionLike"  data-id="${question.questionId}"  data-count="${question.goodCount}" onclick="question_like(this,${question.questionId})"><i class="glyphicon glyphicon-thumbs-up"></i><br>
        ...
        <div class="goods-detail-null-content" style="display:${empty questionVOPage.records ? 'block':'none'}" id="J_commentTipInfo">
          <h3>暂时还没有更多提问</h3>
          <p>对商品还不太了解，问问看吧</p>
        </div>
        <div class="comment-page-nav-list" id="J_issuePagenav">
          <div class="xm-pagenavi">
            <c:if test="${questionVOPage.total gt questionVOPage.size and not empty questionVOPage.records}">
              <div id="pager" data-pager-href="${ctx}/question/list?productId=${productVO.productId}&sort=${sort}&page=" data-pager-totalPage="${totalPage}" data-pager-current="${questionVOPage.current}"></div>
            </c:if>
          </div>
        </div>
      </div>
      <div class="span6 goods-detail-info-block">
        <div class="goods-detail-info">
          <div class="goods-img-block"> <a target="_blank" href="${ctx}/detail/${productVO.productId}" title="${productVO.name}"> <img src="${ctximg}/${productVO.picImg}" class="J_cartBigImg" alt="${productVO.name}"> </a> </div>
          <div class="goods-name J_cartGoodsName"> ${productVO.name} </div>
          <div class="goods-price"> <b>${productVO.showPrice}</b><i>&nbsp;元</i> </div>
        </div>
        <div class="goods-cart-btn-block" id="J_cartBtnBlock"> <a target="_blank" href="${ctx}/detail/${productVO.productId}" class="btn btn-primary goods-add-cart-btn">加入购物车</a> </div>
      </div>
    </div>
  </div>
</div>
...
<myfooter> 
  ...
  <script type="text/javascript">
	...
	function question_like(obj, questionId) {
		var data = {};
		data.questionId = $(obj).attr("data-id");
		data.goodCount = $(obj).attr("data-count");
		$.ajax({
			type : 'put',
			dataType : 'json',
			data : data,
